...
{
	hipMemcpy((void*)dst, (void*)src, N*sizeof(T), hipMemcpyDeviceToDevice);
}

template <typename T>
void upload(T* dst, T* src, int N)
{
	hipMemcpy((void*)dst, (void*)src, N*sizeof(T), hipMemcpyHostToDevice);
}

template <typename T>
void download(T* dst, T* src, int N)
{
	hipMemcpy((void*)dst, (void*)src, N*sizeof(T), hipMemcpyDeviceToHost);
}
...
__global__ void 
cuda_initialize_variables(int nelr, float* variables, float* ff_variable)
{
...
cuda_compute_step_factor( 
                int nelr, float* variables, float* areas, float* step_factors)
{
	const int i = (hipBlockDim_x*hipBlockIdx_x + hipThreadIdx_x);
...
__global__ void 
cuda_compute_flux( 
                int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes, float* ff_variable, float3* ff_flux_contribution_momentum, float3 *ff_flux_contribution_density_energy)
{
	...
	for(j = 0; j < NNB; j++)
	{
		...
		else if(nb == -2) // a far field boundary
		{
			...
			flux_i_momentum.x += factor*(ff_flux_contribution_momentum[0].x + flux_contribution_i_momentum_x.x);
			flux_i_momentum.y += factor*(ff_flux_contribution_momentum[1].x + flux_contribution_i_momentum_y.x);
			flux_i_momentum.z += factor*(ff_flux_contribution_momentum[2].x + flux_contribution_i_momentum_z.x);

			factor = float(0.5f)*normal.y;
			flux_i_density += factor*(ff_variable[VAR_MOMENTUM+1]+momentum_i.y);
			flux_i_density_energy += factor*(ff_flux_contribution_density_energy[0].y+flux_contribution_i_density_energy.y);
			flux_i_momentum.x += factor*(ff_flux_contribution_momentum[0].y + flux_contribution_i_momentum_x.y);
			flux_i_momentum.y += factor*(ff_flux_contribution_momentum[1].y + flux_contribution_i_momentum_y.y);
			flux_i_momentum.z += factor*(ff_flux_contribution_momentum[2].y + flux_contribution_i_momentum_z.y);

			factor = float(0.5f)*normal.z;
			flux_i_density += factor*(ff_variable[VAR_MOMENTUM+2]+momentum_i.z);
			flux_i_density_energy += factor*(ff_flux_contribution_density_energy[0].z+flux_contribution_i_density_energy.z);
			flux_i_momentum.x += factor*(ff_flux_contribution_momentum[0].z + flux_contribution_i_momentum_x.z);
			flux_i_momentum.y += factor*(ff_flux_contribution_momentum[1].z + flux_contribution_i_momentum_y.z);
			flux_i_momentum.z += factor*(ff_flux_contribution_momentum[2].z + flux_contribution_i_momentum_z.z);
...
int main(int argc, char** argv)
{
  printf("WG size of kernel:initialize = %d, WG size of kernel:compute_step_factor = %d, WG size of kernel:compute_flux = %d, WG size of kernel:time_step = %d\n", BLOCK_SIZE_1, BLOCK_SIZE_2, BLOCK_SIZE_3, BLOCK_SIZE_4);
    ...
    float3* ff_flux_contribution_momentum = alloc<float3>(3);
    float3* ff_flux_contribution_density_energy = alloc<float3>(1);
	{
		...
		float3 h_ff_flux_contribution_momentum[3];
		float3 h_ff_flux_contribution_density_energy;
		compute_flux_contribution(h_ff_variable[VAR_DENSITY], h_ff_momentum, h_ff_variable[VAR_DENSITY_ENERGY], ff_pressure, ff_velocity, h_ff_flux_contribution_momentum[0], h_ff_flux_contribution_momentum[1], h_ff_flux_contribution_momentum[2], h_ff_flux_contribution_density_energy);

		// copy far field conditions to the gpu
		upload<float>(ff_variable, h_ff_variable, NVAR);
		upload<float3>(ff_flux_contribution_momentum, h_ff_flux_contribution_momentum, 3);
		upload<float3>(ff_flux_contribution_density_energy, &h_ff_flux_contribution_density_energy, 1);
	}
	int nel;
	int nelr;
...
#ifdef PROFILING
    PerfSerializer *serializeTime = new SimplePerfSerializer(argv[2]);
    RDTimer *myTimer = new RDTimerGPU();
    myTimer->Reset("COMPUTE: compute_step_factor + compute_flux + time_step");
#endif
	...
	for(int i = 0; i < iterations; i++)
	{
		...
		for(int j = 0; j < RK; j++)
		{
			compute_flux(nelr, elements_surrounding_elements, normals, variables, fluxes, ff_variable, ff_flux_contribution_momentum, ff_flux_contribution_density_energy);
    ...
    dealloc<float>(ff_variable);
    dealloc<float3>(ff_flux_contribution_momentum);
    dealloc<float3>(ff_flux_contribution_density_energy);
